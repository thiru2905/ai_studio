...
export function CodeDocsFullscreen() {
  const { codeDocsDoc, updateCodeDocsTemplate, updateCodeDocsMarkdown } = useAppStore()
    ...
    <div className="flex flex-col h-full" style={{ background: 'var(--bg)' }}>
      <div className="h-10 flex items-center justify-between px-4 shrink-0" style={{ borderBottom: '1px solid var(--border)' }}>
        <div className="flex items-center gap-4 text-xs" style={{ color: 'var(--text-tertiary)' }}>
          ...
          <span>→</span>
          <span className="flex items-center gap-1.5 font-semibold" style={{ color: '#f59e0b' }}><Code2 size={11} /> Markdown</span>
          <span>→</span>
          <span className="flex items-center gap-1.5 font-semibold" style={{ color: 'var(--accent)' }}><Eye size={11} /> Preview</span>
        </div>
        <div className="relative">
          <button onClick={() => setDlOpen(!dlOpen)} disabled={!codeDocsDoc.markdown}
            className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg transition-all text-white"
            style={{ background: codeDocsDoc.markdown ? 'var(--accent)' : 'var(--bg-tertiary)', opacity: codeDocsDoc.markdown ? 1 : 0.4, color: codeDocsDoc.markdown ? 'white' : 'var(--text-tertiary)' }}>
            <Download size={11} /> Export <ChevronDown size={9} />
          </button>
          {dlOpen && (
            <>
              <div className="fixed inset-0 z-40" onClick={() => setDlOpen(false)} />
              <div className="absolute right-0 top-full mt-1.5 rounded-xl py-1 z-50 min-w-[170px] animate-slide-up"
                style={{ background: 'var(--surface-raised)', border: '1px solid var(--border)', boxShadow: 'var(--shadow-lg)' }}>
                {[
                  { icon: <FileText size={11} />, label: 'Plain text (.txt)', fn: () => { downloadAsTxt(codeDocsDoc.markdown, codeDocsDoc.title); setDlOpen(false) } },
                  { icon: <Code2 size={11} />, label: 'Markdown (.md)', fn: () => { downloadAsMd(codeDocsDoc.markdown, codeDocsDoc.title); setDlOpen(false) } },
                  null as null,
        ...
        <PanelGroup direction="horizontal" className="h-full">
          <Panel defaultSize={33} minSize={20}>
            <div className="flex flex-col h-full">
              ...
              <textarea value={codeDocsDoc.template} onChange={e => updateCodeDocsTemplate(e.target.value)}
          ...
          <Panel defaultSize={34} minSize={20}>
            <div className="flex flex-col h-full">
              <div className="h-9 flex items-center px-4 gap-2 shrink-0"
                style={{ background: 'rgba(245,158,11,0.05)', borderBottom: '2px solid rgba(245,158,11,0.2)' }}>
                <Code2 size={12} style={{ color: '#f59e0b' }} />
                <span className="text-xs font-semibold" style={{ color: '#f59e0b' }}>Markdown Source</span>
              </div>
              <textarea value={codeDocsDoc.markdown} onChange={e => updateCodeDocsMarkdown(e.target.value)}
                className="flex-1 w-full resize-none outline-none p-4 font-mono leading-relaxed"
                style={{ fontSize: '11px', background: 'var(--bg)', color: 'var(--text-secondary)' }}
                placeholder="Documentation will appear here..." spellCheck={false} />
            </div>
          </Panel>
          <Divider />
          <Panel defaultSize={33} minSize={20}>
            <div className="flex flex-col h-full">
              <div className="h-9 flex items-center px-4 gap-2 shrink-0"
                ...
                <span className="text-xs font-semibold" style={{ color: 'var(--accent)' }}>Preview</span>
              </div>
              <div className="flex-1 overflow-y-auto">
                {codeDocsDoc.markdown ? (
                  ...
                  <div className="flex items-center justify-center h-full">
                    <p className="text-xs text-center px-4" style={{ color: 'var(--text-tertiary)' }}>Documentation preview appears here.</p>
                  </div>
                )}
              </div>
            </div>
